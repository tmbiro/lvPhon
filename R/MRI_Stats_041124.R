...
# Check which random effects structure best explains the variability in the data 
ma <- lmer(value ~ 
             Gyrus 
           * Hemisphere
           * Phon_Acc
           + (1 | PID) # Random intercept of PID
           , data = volDat) 
mb <- lmer(value ~ 
            Gyrus 
          * Hemisphere
          * Phon_Acc
          + (1 | PID) # Random intercept of PID
          + (0 + Gyrus | PID) # Random slope of Gyrus on PID, this will take a long time to run, so use gaming PC
          , data = volDat) 
